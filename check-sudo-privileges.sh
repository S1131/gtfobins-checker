...
do
    if echo $line | egrep -s -e 'User ([a-z_]+) may run the following commands on ([a-z_]+):'; then                
        user=$(echo $line | awk '{print $2}')
        IFS= read -r line
        while echo "$line" | egrep -s -e '^    '
        do            
            if echo $line | grep -s "NOPASSWD:" ;then

                commands=$(echo $line | awk -F 'NOPASSWD:' '{print $2}')
                read_nopass_commands "$commands"

            fi
            if echo $line | grep -s "NOPASSWD: ALL" ;then
